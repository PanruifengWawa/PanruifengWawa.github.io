function addMetroGenral(){
  ...
  addQuestion({
    ...
    add:wrpperAdd(geneNumberItem("如果采用公交车上班，单程：</br>较快情况下大概_分钟</br>较慢情况下大概_分钟")+
    ...
    if(hasCar){
      ...
      senario.last().each(function(item,i){
        addQuestion({
          ...
          title:(item[0]-getCost()) ==0 ? "您家到工作地点，有新自驾线路"+(i+1)+"开通（信息如下），您会选择？" :"您家到工作地点，假设现在地铁票价政策性提高"+(item[0]-getCost())+"元，并有新自驾线路开通"+(i+1)+"开通（信息如下），您会选择？",
          list:["原选择（地铁）","新自驾线路"+(i+1)],
          size:"two",
          subTitle:"场景"+(i+1)+"：<span class='red'>"+((i+1>1)?"(此场景不同于以上场景)":"")+"<br/></span>",
          answerId:false
        });
