...
function addCarGeneral(){
  ...
  addQuestion({
    ...
    list:["较为方便","可以但不太方便（家或公司离公交站远/需要多次换乘/时间太长等原因）","不可以"],
...
function appendCarSenario4(){
  if(checkInput()){
    addSenaAnswer();
    nextPage();
    addText("部分3：新路线情景题<br/>（请综合考虑出行距离、费用、时间范围等因素做出选择）","red");
...
function appendCarSenario3Content(item,i){
  ...
  J.id("quesWrapper").child().last().append(wrapperSena3(
    geneSenario34(item[0],transType,c0times,undefined,item[2])
...
function geneSenario34(cost,type,x,i,mean){
  var ctext=["有座位","站立，不拥挤","","站立，很拥挤"];
  var costText=(type=="car")?"费用":"票价";
