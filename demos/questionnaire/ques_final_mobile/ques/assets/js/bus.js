...
function addBusGeneral(){
  ...
  addQuestion({
    ...
    add:wrpperAdd(geneNumberItem("如果采用地铁上班，单程：</br>较快情况下大概_分钟大概_分钟</br>较慢情况下大概_分钟")+
  ...
  if(checkInput()){
    ...
    if(hasCar){
      ...
      senario.last().each(function(item,i){
        addQuestion({
          type:"single",
          title:"从您家到工作地点，有新开车线路"+(i+1)+"开通（信息如右侧），您回选择？",
      ...
      senario.last().each(function(item,i){
        addQuestion({
          type:"single",
          title:"从您家到工作地点，有新开车线路"+(i+1)+"开通（信息如右侧），您会选择？",
...
function appendBusSenarioNew(){
  hideLotter();
  nextPage();
  addText("部分3：新工作情景题<br/>请综合考虑场景中费用、时间范围、车内拥挤程度等因素做出选择","red");
...
function appendBusAndSenarioNewContent(item,i){
  ...
  J.id("quesWrapper").child().last().append(wrapperSena3(
    geneSenario34(item[0],type,c0times,undefined,item[2])
    +geneSenario34(item[3],"metro",ctimes,item[6],item[5])
    +geneSenario34(item[7],"bus",c2times,item[10],item[9])
  ));
