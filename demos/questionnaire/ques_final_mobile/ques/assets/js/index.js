...
function geneTimeEle(x){
  ...
  x.each(function(a){
    if(Math.round(a[2]*100)!=0&&i<6){
      i++;
      s+='<div class="new-time">'+Math.round(a[0])+'~'+Math.round(a[1])+'分钟的可能性'+Math.round(a[2]*100)+'%</div>';
    }
  });
  s+='</div>';
  if(i<6){
...
function geneTimeEle2(x){
  var s='<div class="s-item-wrapper">';
  var i=0;
  x.each(function(a){
    if(Math.round(a[2]*100)!=0&&i<6){
      i++;
      s+='<div class="new-time">'+Math.round(a[0])+'~'+Math.round(a[1])+'的可能'+Math.round(a[2]*100)+'%</div>';
    }
  });
  s+='</div>';
  if(i<6){
    ...
    if(canLot){
      ...
      setTimeout(function(){
        ...
        if(_lot_num==1){
          ...
          if(money>0){
            obj.next().html('恭喜您中奖了！本次您中了<span class="red money">'+money+'</span>元！两次抽奖共'+(money+last_money)+'元。请填写您的支付宝账号和姓名方便转账</br>\
